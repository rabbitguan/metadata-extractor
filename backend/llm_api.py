...
def _build_prompt(content, standard, url='', title='', preclassified_type=None):
    ...
    rules = "\n".join([
        "请严格遵守以下规则:",
        "",
        "1) 返回严格 JSON，顶层必须是统一元数据对象，不要包含 \"zh\" / \"en\" 顶层分组，不要输出解释文本或多余字符。",
        "",
        "【幻觉防控 - 严格遵守】:",
        ...
        "",
        "9) 标识符类型要求：对于核心字段中的 CSTR 标识符，必须是 CSTR 格式（示例：12345.12.123456.123456）。如果网页中没有明确的 CSTR 标识符，请返回 null，不要用 arXiv ID、DOI 或 URL 代替。",
        "",
        "10) 必须从\"资源类型候选列表\"中判断资源类型，并将结果写入核心字段 resource_type，值使用规范英文枚举：\"Dataset\"/\"Data Paper\"/\"Other\"。",
        "",
        "11) 核心元数据对象只能包含上述 17 个字段，不要把 domain_metadata、领域判定、extension_info、扩展信息放入核心元数据对象。",
        "",
        "12) 除了核心元数据外，还要根据 resource_type 返回对应的完整领域 schema 结构：",
        ...
        "",
        "13) 如果页面来自 arXiv（URL/title/text 中出现 arXiv 线索），优先判定为\"数据论文\"。",
        "",
        "14) 最终返回的 JSON 顶层结构应该是统一对象，不要包含顶层 zh/en：",
        "    {",
        '        "核心元数据": {"metadatas": [{',
