...
def resolve_doi(doi, clean_html=None):
    try:
        ...
        print(f"[WARNING] Crossref failed for {doi}, falling back to DOI landing page: {crossref_error}")
        try:
            return _fetch_landing_page(doi, clean_html)
        except Exception as landing_error:
            raise ValueError(
                f"Failed to resolve DOI {doi}; Crossref error: {crossref_error}; "
                f"doi.org error: {landing_error}"
            ) from landing_error
